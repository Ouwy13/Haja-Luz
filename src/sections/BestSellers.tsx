...
export default function BestSellers() {
  ...
  }, [isPaused, isOverlayOpen, bestSellerProducts.length]);
    ...
    <section id="mais-vendidos" className="py-12 bg-[#f3f4f6] overflow-hidden">
      <ScrollReveal blur="12px" yOffset={30} duration={0.7} className="w-full">
        ...
        >
          {/* Navigation Arrows */}
          <button 
            onClick={() => setCurrentIndex((prev) => (prev - 1 + bestSellerProducts.length) % bestSellerProducts.length)}
          ...
          <button 
            onClick={() => setCurrentIndex((prev) => (prev + 1) % bestSellerProducts.length)}
