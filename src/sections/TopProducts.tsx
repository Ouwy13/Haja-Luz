      ...
      <ScrollReveal blur="12px" yOffset={30} duration={0.7} className="w-full">
        <div className="max-w-[1240px] mx-auto px-4 flex items-center justify-between mb-8 gap-4">
          <div className="flex items-center gap-3">
            ...
            <span className="bg-gradient-to-r from-[#EE4D2D] to-[#FF3B62] text-white text-[10px] md:text-xs py-1 px-2.5 rounded-full font-bold uppercase tracking-wider flex items-center gap-1 shadow-sm">
        ...
        >
          ...
          <motion.div 
            ...
            dragConstraints={{
              right: 0,
              left: -((topProducts.length * 3) * itemWidth - (typeof window !== 'undefined' ? window.innerWidth : 1200))
            }}
            onDragEnd={(_, info) => {
              if (info.offset.x < -50) setCurrentIndex(prev => (prev + 1) % topProducts.length);
              if (info.offset.x > 50) setCurrentIndex(prev => (prev - 1 + topProducts.length) % topProducts.length);
            }}
            animate={{
          ...
          >
            ...
            {[...topProducts, ...topProducts, ...topProducts].map((product, idx) => (
              <div 
                key={`${product.id}-${idx}`} 
            ...
            <div className="bg-[#1C2978] text-white px-4 md:px-8 h-16 md:h-20 flex items-center shadow-md flex-shrink-0 w-full relative z-10">
              <div className="flex items-center w-full">
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div>
                    <h2 className="font-bold text-lg md:text-xl truncate">Destaques da Semana</h2>
                    <p className="text-[10px] md:text-xs text-blue-100 font-medium uppercase tracking-wider opacity-70">Produtos que você não pode perder</p>
                  </div>
                </div>
              ...
              <div className="w-full py-8">
                ...
                <div className="mb-8 flex justify-between items-center text-[#6B7890] text-sm font-semibold tracking-wide">
                  <span className="bg-white px-4 py-1.5 rounded-full border border-slate-100 shadow-sm flex items-center gap-2">
                    ...
                    Seleção exclusiva dos produtos mais procurados
                  </span>
                </div>
